...
from typing import Any, Dict, List, Optional, Set, Tuple, ValuesView, cast
...
# Input names for Normal map connections
NORMAL_INPUT_NAMES = ["Normal", "Normal Map"]

# Input names for ORM components
ORM_INPUT_NAMES = {
    "roughness": ["Roughness"],
    "metallic": ["Metallic"],
    "occlusion": ["Occlusion", "Ambient Occlusion"]
}
...
def get_material_images(mat: bpy.types.Material) -> Dict[str, Any]:
    """Extracts all relevant images/values from a material.

    Args:
        mat: Material to extract data from.

    Returns:
        Dictionary containing 'diffuse', 'normal', and 'orm' data.
    """
    result = {
        "diffuse": get_image_from_material(mat),
        "normal": get_normal_image(mat),
        "orm": get_orm_components(mat),
    }
    return result


def get_normal_image(mat: bpy.types.Material) -> Optional[bpy.types.Image]:
    """Extracts the normal map image from a material.

    Args:
        mat: Material to extract normal map from.

    Returns:
        The normal map image or None if not found.
    """
    if not mat.node_tree or not mat.node_tree.nodes:
        return None
    ...
    # Try to find output node and trace back
    output_node = _find_output_node(node_tree.nodes)
    if output_node:
        shader_result = _trace_connected_shader(output_node)
        if shader_result:
            shader_node, _ = shader_result
            
            # Check Normal input
            if "Normal" in shader_node.inputs:
                input_socket = shader_node.inputs["Normal"]
                if input_socket.links:
                    link = input_socket.links[0]
                    node = link.from_node
                    
                    # Usually connected to a Normal Map node first
                    if node.bl_idname == "ShaderNodeNormalMap":
                        if "Color" in node.inputs and node.inputs["Color"].links:
                            color_link = node.inputs["Color"].links[0]
                            if _is_image_texture_node(color_link.from_node):
                                return color_link.from_node.image
                    
                    # Or directly connected (less common but possible)
                    elif _is_image_texture_node(node):
                        return node.image
                        
    return None


def get_orm_components(mat: bpy.types.Material) -> Dict[str, Any]:
    """Extracts ORM (Occlusion, Roughness, Metallic) components.

    Args:
        mat: Material to extract ORM data from.

    Returns:
        Dictionary with keys 'occlusion', 'roughness', 'metallic',
        containing either an Image or a float value.
    """
    components = {
        "occlusion": 1.0,  # Default white (no occlusion)
        "roughness": 1.0,  # Default white (fully rough)
        "metallic": 0.0,   # Default black (non-metallic)
    }
    ...
    if output_node:
        ...
        if shader_result:
            ...
            # Helper to get image or value from socket
            def get_socket_data(socket_name):
                if socket_name in shader_node.inputs:
                    socket = shader_node.inputs[socket_name]
                    if socket.links:
                        # Follow the link to find an image
                        node = socket.links[0].from_node
                        # Direct connection
                        if _is_image_texture_node(node):
                            return node.image
                        # Indirect connection (e.g. Separate Color)
                        # This is a simplification; for robust ORM packing we might need 
                        # to know WHICH channel is used. For now, we return the image
                        # and let the packer handle it (it will use the image as grayscale).
                        # Ideally, we should track (Image, Channel).
                        img_node = _find_image_texture_in_node_tree(node, set())
                        if img_node:
            ...
            # Get Roughness
            r_data = get_socket_data("Roughness")
            if r_data is not None:
                components["roughness"] = r_data
                
            # Get Metallic
            m_data = get_socket_data("Metallic")
            if m_data is not None:
                components["metallic"] = m_data
                
            # Get Occlusion (standard Principled BSDF doesn't have it, but custom groups might)
            # Or if using glTF Separator
            # We check for "Occlusion" input just in case
            o_data = get_socket_data("Occlusion") 
            if o_data is not None:
                components["occlusion"] = o_data
