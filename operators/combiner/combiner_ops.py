...
def get_atlas(scn: Scene, data: Structure, atlas_size: Tuple[int, int]) -> Dict[str, ImageType]:
    """Generate the texture atlas images.
    ...
    Returns:
        Dictionary of generated atlas images.
    """
    smc_size = (scn.smc_size_width, scn.smc_size_height)
    half_gaps = int(scn.smc_gaps / 2)
    
    atlases = {
        'diffuse': Image.new('RGBA', atlas_size),
        'normal': Image.new('RGBA', atlas_size),
        'orm': Image.new('RGBA', atlas_size)
    }

    for mat, item in data.items():
        _set_image_or_color(item, mat)
        for map_type, img in atlases.items():
            _paste_gfx(scn, item, mat, img, half_gaps, map_type)
    ...
    for map_type, img in atlases.items():
        ...
        if scn.smc_size == 'STRICTCUST':
            ...
            result_atlases[map_type] = canvas_img
        else:
            result_atlases[map_type] = img

    return result_atlases
...
def _set_image_or_color(item: StructureItem, mat: bpy.types.Material) -> None:
    ...
    if is_blender_modern:
        ...
        # Diffuse
        diffuse_img = images.get('diffuse')
        item['gfx']['images']['diffuse'] = get_packed_file(diffuse_img) if diffuse_img else None

        # Normal
        normal_img = images.get('normal')
        item['gfx']['images']['normal'] = get_packed_file(normal_img) if normal_img else None

        # ORM
        orm_data = images.get('orm', {})
        item['gfx']['images']['orm'] = {}
        for key in ['occlusion', 'roughness', 'metallic']:
            val = orm_data.get(key)
            if val and hasattr(val, 'packed_file'):
                item['gfx']['images']['orm'][key] = get_packed_file(val)
            else:
                item['gfx']['images']['orm'][key] = val

        item['gfx']['img_or_color'] = item['gfx']['images']['diffuse']
    else:
        item['gfx']['img_or_color'] = get_packed_file(get_image(get_texture(mat)))
        item['gfx']['images'] = {'diffuse': item['gfx']['img_or_color']}

    if not item['gfx']['img_or_color']:
        diffuse_color = get_diffuse(mat)
        item['gfx']['img_or_color'] = diffuse_color
        if 'images' in item['gfx']:
            item['gfx']['images']['diffuse'] = diffuse_color


def _paste_gfx(scn: Scene, item: StructureItem, mat: bpy.types.Material, img: ImageType, half_gaps: int, map_type: str = 'diffuse') -> None:
    ...
    Args:
        ...
        map_type: Type of map to paste.
    """
    if not item['gfx']['fit']:
        return

    img.paste(
        _get_gfx(scn, mat, item, item['gfx']['images'].get(map_type), map_type),
...
def _get_gfx(scn: Scene, mat: bpy.types.Material, item: StructureItem,
             img_or_color: Union[bpy.types.PackedFile, Tuple, Dict, None], map_type: str = 'diffuse') -> ImageType:
    ...
    Args:
        ...
        item: Material metadata.
        img_or_color: Image data, color tuple, or ORM dict.
    ...
    if not img_or_color and map_type != 'orm':
        if map_type == 'normal':
            return Image.new('RGBA', size, (128, 128, 255, 255))
        return Image.new('RGBA', size, (255, 255, 255, 255))

    if map_type == 'orm':
        return _get_orm_gfx(scn, mat, item, img_or_color, size)
    ...
    if map_type == 'diffuse' and mat.smc_diffuse:
...
def _get_orm_gfx(scn: Scene, mat: bpy.types.Material, item: StructureItem, orm_data: Dict, size: Tuple[int, int]) -> ImageType:
    """Generate ORM image from components."""
    defaults = {
        'occlusion': 1.0,
        'roughness': 1.0,
        'metallic': 0.0
    ...
    channels = []
    # ORM: Red=Occlusion, Green=Roughness, Blue=Metallic
    for key in ['occlusion', 'roughness', 'metallic']:
        val = orm_data.get(key)
        if val is None:
            val = defaults[key]
        ...
        if isinstance(val, (int, float)):
             ch_img = Image.new('L', size, int(val * 255))
        else:
             # PackedFile
             ...
             if max(item['gfx']['uv_size'], default=0) > 1:
                 ch_img = _get_uv_image(item, ch_img, size)
...
def get_comb_mats(scn: Scene, atlases: Dict[str, ImageType], mats_uv: MatsUV) -> CombMats:
    """Create materials for the generated atlas.

    Args:
        scn: Current scene.
        atlases: Generated atlas images.
        mats_uv: Dictionary mapping object names to materials with UV coordinates.
    ...
    textures = {}
    for map_type, atlas in atlases.items():
        suffix = {'diffuse': '_D', 'normal': '_N', 'orm': '_ORM'}.get(map_type, '')
        path = _save_atlas(scn, atlas, unique_id, suffix)
        textures[map_type] = _create_texture(path, unique_id, map_type)

    return cast(CombMats, {idx: _create_material(textures, unique_id, idx) for idx in layers})
...
def _add_ids_from_existing_files(scn: Scene, existed_ids: Set[int]) -> None:
    ...
    Args:
        ...
        existed_ids: Set to add IDs to.
    """
    atlas_file_pattern = re.compile(r'{}(\d+)(_.*)?.png'.format(atlas_prefix))
...
def _save_atlas(scn: Scene, atlas: ImageType, unique_id: str, suffix: str = '') -> str:
    ...
    """
    path = os.path.join(scn.smc_save_path, '{}{}{}.png'.format(atlas_prefix, unique_id, suffix))
    atlas.save(path)
    print(f"[SMC Log] Saved atlas: {path}")
    return path


def _create_texture(path: str, unique_id: str, map_type: str) -> bpy.types.Texture:
    ...
    """
    texture = bpy.data.textures.new('{}{}_{}'.format(atlas_texture_prefix, unique_id, map_type), 'IMAGE')
    image = bpy.data.images.load(path)
    texture.image = image
    return texture


def _create_material(textures: Dict[str, bpy.types.Texture], unique_id: str, idx: int) -> bpy.types.Material:
    ...
    if is_blender_modern:
        _configure_material(mat, textures)
    else:
        _configure_material_legacy(mat, textures.get('diffuse'))
    return mat


def _configure_material(mat: bpy.types.Material, textures: Dict[str, bpy.types.Texture]) -> None:
    ...
    # Principled BSDF
    node_bsdf = mat.node_tree.nodes.new(type='ShaderNodeBsdfPrincipled')
    ...
    if 'diffuse' in textures:
        ...
        tex_node.image = textures['diffuse'].image
        tex_node.label = 'Diffuse'
        tex_node.location = -300, 300
        mat.node_tree.links.new(tex_node.outputs['Color'], node_bsdf.inputs['Base Color'])
        mat.node_tree.links.new(tex_node.outputs['Alpha'], node_bsdf.inputs['Alpha'])

    # Normal
    if 'normal' in textures:
        tex_node = mat.node_tree.nodes.new(type='ShaderNodeTexImage')
        tex_node.image = textures['normal'].image
        ...
        norm_map = mat.node_tree.nodes.new(type='ShaderNodeNormalMap')
        norm_map.location = -100, 0
        
        mat.node_tree.links.new(tex_node.outputs['Color'], norm_map.inputs['Color'])
        mat.node_tree.links.new(norm_map.outputs['Normal'], node_bsdf.inputs['Normal'])

    # ORM
    if 'orm' in textures:
        tex_node = mat.node_tree.nodes.new(type='ShaderNodeTexImage')
        tex_node.image = textures['orm'].image
        tex_node.label = 'ORM'
        tex_node.location = -300, -300
        try:
            tex_node.image.colorspace_settings.name = 'Non-Color'
        ...
        try:
             sep_rgb = mat.node_tree.nodes.new(type='ShaderNodeSeparateColor')
        except RuntimeError:
             sep_rgb = mat.node_tree.nodes.new(type='ShaderNodeSeparateRGB')
             
        sep_rgb.location = -100, -300
        
        mat.node_tree.links.new(tex_node.outputs['Color'], sep_rgb.inputs[0]) 
        
        # Green -> Roughness
        mat.node_tree.links.new(sep_rgb.outputs[1], node_bsdf.inputs['Roughness'])
        # Blue -> Metallic
        mat.node_tree.links.new(sep_rgb.outputs[2], node_bsdf.inputs['Metallic'])
